...
def process_chunk(chunk, q, detector):
    ...
    for line in chunk.split('\n'):
        ...
        if line.startswith('  fs/px   ss/px'):
            ...
            s = line.split()
            reflections.append([float(s[-3]), float(s[-2]), int(s[0]), int(s[1]), int(s[2]), va, vb, vc, s[-1]])
    ...
    for p in peaks:
        ...
        if nr and np.sqrt(d) < 3:
            ...
            xp = ((p[0] - panel['min_fs']) * panel['fsx'] + (p[1] - panel['min_ss']) * panel['ssx'] + panel['cnx'])/panel['res']
            yp = ((p[0] - panel['min_fs']) * panel['fsy'] + (p[1] - panel['min_ss']) * panel['ssy'] + panel['cny'])/panel['res']
            dp = panel['clen']

            k_peak.append(h/np.sqrt(2*(1 - dp/np.sqrt(dp**2 + xp**2 + yp**2))) * to_ev)
            r_peak.append(h)
...
def parse_args():
    parser = argparse.ArgumentParser(description='', usage='./peaks_hist.py stream geometry spectrum [options]')
    ...
    parser.add_argument('-n', '--nproc', metavar='N', type=int, default=1, help="run N processes in parallel, default = 1")
...
if __name__ == '__main__':
    ...
    y, bins, patches = ax2.hist(k_peak[np.where(d_peak < 30)], 100, alpha=0.6, color='C2', label='Found peaks') 

    ax2.set_ylabel('N peaks', color='C2') 
    for tl in ax2.get_yticklabels(): 
        tl.set_color('C2') 

    x = [(bins[i] + bins[i+1]) / 2. for i in range(y.shape[0])]

    p = curve_fit(fitfunc, x, y, p0=((y.max()/1., 1)))
    sf = 1/p[0][1]
