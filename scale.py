        ...
        if line.startswith('----- Begin chunk -----'):
            ...
            if not reading_reflections:
                ...
                if line.startswith('End of reflections'):
                    reading_reflections = False
                    chunk = chunk.replace(num_refl_line, "num_reflections = %d"%(nt - nr))
                    #print(nr, nt, r)
                else:
                    nt += 1
                    ...
                    kmin = -np.linalg.norm(h)**2/2/(h[2] - r) * m_to_ev - r * m_to_ev
                    kmax = -np.linalg.norm(h)**2/2/(h[2] + r) * m_to_ev + r * m_to_ev
                    w = 0
                    n = 0
                    k = 0
                    ...
                    for i in range(len(ks) - 1):
                        if ks[i] < k <= ks[i + 1]:
                            w = ((ks[i + 1] - k) * ws[i] + (k - ks[i]) * ws[i + 1])/(ks[i + 1] - ks[i])
                    ...
                    if w/maxw >= minw and line.split()[3] not in ('nan', '-nan'):
                        ...
                        chunk = chunk.replace(line, '%4d %4d %4d %10.2f %10.2f %10.2f %10.2f %6.1f %6.1f %s'%tuple((float(i) if i[0] != 'p' else i for i in spl)))
                    else:
                        nr += 1
...
def parse_args():
    parser = argparse.ArgumentParser(description='', usage='./scale.py stream spectrum [options]')
    parser.add_argument('stream', type=str, help="stream file")
    parser.add_argument('spectrum', type=str, help="spectrum file")

    parser.add_argument('-n', '--nproc', metavar='N', type=int, default=1, help="run N threads in parallel, default = 1")
    
    parser.add_argument('-u', '--unit-cell-scale', metavar = 1., type=float, default=1., help="unit cell scaling factor, default = 1.")
    parser.add_argument('-m', '--min-spectral-weight', metavar = 0.2, type=float, default=None, help="relative spectral weight cut-off, default = 0.2")
    parser.add_argument('-r', '--rdco', metavar='rdco.dat', type=str, default=None, help="resolution dependant spectral weight cut-off")          
